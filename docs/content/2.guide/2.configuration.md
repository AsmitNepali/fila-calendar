...
The icon defaults to `heroicon-m-bookmark`. Pass any icon name Filament can render:
...
A closure works too, so the icon can follow the record:

```php
CalendarInput::make('availabilities')
    ->reservedDates(fn (): array => $this->getRecord()->bookedDates())
    ->reservedIcon(fn (): string => $this->getRecord()->isProvisional()
        ? 'heroicon-m-clock'
